...
- 时间导航新增「计划」分组：明天/本周/以后作为分组内的分类条件缩进展示，
  导航顺序调整为 已逾期 → 今天 → 计划 → 未安排

### Changed

- 时间桶「待重新安排」更名为「已逾期」，桌面端时间视图、看板角标与移动端
  计划页同步
- 事项详情浮窗的「编辑」开关改为滑动开关样式；原生 checkbox 仅视觉隐藏，
  键盘与读屏操作不受影响

### Fixed

- 设置页标题区复用主窗口 `WindowTitlebar`（自绘窗口控制按钮与拖拽区），
  修复 3.x 自绘标题栏下设置页没有最小化/最大化/关闭按钮的问题

## [v3.6.0] - 2026-09-12

### Added

- 项目导出为图片时逐条渲染事项描述详情（Markdown，与事项详情浮窗的
  预览一致），不再截断为单行摘要；是否显示可在「设置 → 通用 → 图片导出」
  开关，默认开启
...
- 项目导出为图片时，底部署名附 GitHub 仓库链接与二维码（矢量渲染，任意
  主题下可扫描）；是否显示可在「设置 → 通用 → 图片导出」开关，默认开启

### Removed

- 仓库移除 2.x Electron 旧版源码与发版线，只维护 3.x。已发布的 2.x 安装包
  仍可下载；2.x 旧数据可由 3.x 首次启动导入承接；2.x 不再收到更新

## [v3.4.2] - 2026-09-08

### Added

- 贴图事项悬浮提示改为显示完整标题，不再按宽度截断

### Fixed

- 事项详情浮窗首次打开时长标题只显示一行、第二次打开才正常：标题草稿
  异步落位与自适应高度测量存在竞态，改为草稿变化后重新测量；超长标题
  超过最大高度后改为输入框内部滚动，不再被截断
- 修复主窗口最小化后脏坐标（Windows 的 -32000,-32000 哨兵值）被写入
  window-state.json，下次启动窗口被还原到屏幕外、表现为「启动后看不到
  窗口」：最小化期间不再持久化窗口状态，启动还原时过滤离屏/异常尺寸，
  回落到默认居中位置

### Changed

- 事项详情标题增加可编辑的视觉提示：hover/focus 出现淡底，编辑时字重
  略降、光标变 accent 色
