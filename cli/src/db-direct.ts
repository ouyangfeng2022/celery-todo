/**
 * @file 直连模式数据访问层（better-sqlite3）
 * @description GUI 未运行时，CLI 直接读写 SQLite 文件的实现。
 *
 * 这是双模式 CLI 的「离线回退」路径。当 GUI 运行时，db.ts 会改走 IPC
 * 调用渲染进程的 store action（数据实时反映到 GUI），不走本模块。
 *
 * 设计要点：
 ...
 *   故仅当 GUI 未运行（detectMode 返回 'direct'）时才使用本路径。
