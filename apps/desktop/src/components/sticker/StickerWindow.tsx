...
export function StickerWindow({ stickerId, initialProjectId }: Props) {
  ...
  const refresh = useCallback(async () => {
    const ps = await data.getProjects();
    setProjects((prev) => (sameProjects(prev, ps) ? prev : ps));
    const id = projectIdRef.current || ps[0]?.id || '';
    setTodos(await loadStickerTodos(id));
  }, []);
  useEffect(() => {
    void (async () => {
      // 同步读取本窗口应有的样式设置（首次加载 / 老数据缺失键时走默认）
      await useSettingsStore.getState().loadSettings({ syncStartupTheme: false });
      ...
      if (!projectIdRef.current && ps[0]) {
        ...
        setStickerProject(stickerId, ps[0].id);
      }
      await refresh();
      setReady(true);
    })();
  ...
  useEffect(() => {
    const off = onStickerStyleChanged(() => {
  ...
  useEffect(() => {
    // v3：跨窗口写命令由 Rust 广播 data-changed，平台层已过滤自发事件；
    // 本窗口直接重读当前项目（每写一行都即时提交，无补丁合并的必要）。
  ...
  const handleAdd = async (titles: string[], priority: Priority) => {
    ...
    const existing = await data.getTodos(pid);
  ...
  const contextMenuItems: ContextMenuItem[] = [
    {
      ...
      disabled: !projectId,
      onClick: () => {
        duplicateSticker(stickerId, projectId);
      },
    },
    { separator: true },
    {
    ...
    >
      <header className="sticker-drag sticker-header" data-tauri-drag-region>
        ...
        <button
          ...
          aria-label="返回主窗口"
          title="返回主窗口"
          onClick={() => returnToMain(stickerId)}
        >
        ...
        <div className="sticker-header-actions sticker-no-drag">
          ...
          <button
            ...
            aria-label="关闭贴图"
            onClick={() => closeSticker(stickerId)}
          >
            <svg viewBox="0 0 12 12" aria-hidden="true">
